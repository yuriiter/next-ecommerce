...
try {
  ...
  for (let i = 0; i < 20; i++) {
    reviews.push({
      user: randomItemInArray(Object.values(userIds)),
  ...
  for (let i = 0; i < 20; i++) {
    cars.push({
      ...
      carType: randomItemInArray(carTypes),
      fuelCapacity: randomItemInArray([30, 40, 50, 60]),
      peopleCapacity: randomItemInArray(carCapacities),
      isManual: Math.random() >= 0.5,
      price: Math.floor(Math.random() * 50) + 20,
      previousPrice: Math.floor(Math.random() * 50) + 70,
      recommendedFlag: Math.random() >= 0.5,
      popularFlag: Math.random() >= 0.5,
      ...
      rentalData: Array(Math.floor(Math.random() * 4)).fill(0).map(() => {
        ...
        return ({
          pickUpData: {
            dateTime: randomDate(),
            location: randomItemInArray(locations),
          },
          dropOffData: {
  ...
  const carIdsArray = Object.values(carInsertResults.insertedIds)
  carIdsArray.forEach((carId) => {
    const allReviewIds = Array.from({ length: Math.floor(Math.random() * 6) }, () => {
      return randomItemInArray(Object.values(reviewInsertResults.insertedIds))
    })
